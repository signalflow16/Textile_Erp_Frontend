...
import { CustomerSetupForm } from "@/modules/initial-setup/components/CustomerSetupForm";
import { defaultCustomerValues } from "@/modules/initial-setup/constants/defaultValues";
...
export function CustomerSetupPageContent() {
  const statusQuery = useInitialSetupStatus();
  const countsQuery = useMasterCounts();
  const readinessQuery = useSetupReadiness();
  const [values, setValues] = useInitialSetupDraft("initial-setup-customers", defaultCustomerValues);
  ...
  return (
    <div className="page-stack">
      ...
      <CustomerSetupForm
        ...
        status={statusQuery.status?.sections?.customers}
